(() => {
  ...
  document.addEventListener('DOMContentLoaded', (event) => {
    ...
    // const submitButton = document.getElementById('submitPassBtn');
    ...
    function handleSubmitButtonClick() {
      const password = document.getElementById('hbePass').value;
      getKeyMaterial(password).then(keyMaterial => {
        return Promise.all([
          getHmacKey(keyMaterial),
          getDecryptKey(keyMaterial),
          getIv(keyMaterial)
        ]);
      }).then(([hmacKey, decryptKey, iv]) => {
        return decrypt(decryptKey, iv, hmacKey).then(result => {
          console.log(`Decrypt result: ${result}`);
          if (result) {
            return Promise.all([
    ...
    function hbeLoader() {
      ...
      document.getElementById('submitPassBtn').addEventListener('click', handleSubmitButtonClick);
      test();
    }
